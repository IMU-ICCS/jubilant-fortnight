...
public class App 
{
    ...
    public static void main( String[] args ) throws Exception
    {
		...
		for (int w=0; w<numOfWorkers; w++) {
			workers[w] = startWorker(w, activeCounter, evalIterations, pdp, xacmlRequest);
	...
	private static Thread startWorker(int workerId, AtomicInteger activeCounter, int evalIterations, PDP pdp, String xacmlRequest) {
		Thread worker = new Thread(
			new Runnable() {
				...
				public Runnable setParams(int workerId, int evalIterations, PDP pdp, String xacmlRequest) {
			...
			.setParams(workerId, evalIterations, pdp, xacmlRequest)
		);
		worker.start();
		return worker;
	}
